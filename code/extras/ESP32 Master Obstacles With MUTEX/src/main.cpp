...
void loop() {
  ...
  if (millis() > prev_ms_tele + 100)
  {
   ...
   /*
    ...
    long posXLong = long(loop_xPosition);
    long posYLong = long(loop_yPosition);
  ...
  case e::Arrancar:
    ...
    if (loop_xPosition*turnSense <= startXposition*turnSense - 150)
    {
      setSpeed(0);
      delay(10);
  ...
  break;
  case e::EntradaFase3:
    if (loop_yPosition <= 1650-500*turnClockWise)
    {
      KPActual = positionKP;
      KDActual = positionKD;
      setSpeed(0);
  ...
  */
  ...
  case e::Aparcar1:
    if (loop_yPosition >= 1370 + 530 - 530*turnClockWise)
    {
      setSpeed(0);
      pidEnabled = false;
  ...
  if (MIT < millis() - prev_ms_mainloop){
    MIT = millis() - prev_ms_mainloop;
    prev_ms_mainloop = millis();
...
void iteratePositionPID() {
  ...
  if (fixXposition) {
    positionError = directionError(loop_xPosition, objectivePosition);
  } else {
    positionError = directionError(loop_yPosition, objectivePosition);
  }
...
void checkTurn() {
  ...
  case -1:
    ...
    if (loop_yPosition >= 1000) changeLane(1);
    break;
  
  case -2:
    if ((loop_yPosition <= 1600) && setCoordTramo(1, 200, 800)) correctLane(1);
    if (loop_yPosition >= 2000) turn();
    if (totalGiros >= 4) {
      isRecognizing = false;
      moveCamera(0);
    }
  ...
  case -3:
    ...
    if (loop_xPosition >= 1000) changeLane(3);
    break;

  case -4:
    if ((loop_xPosition <= 1600) && (setCoordTramo(3, 2800, 2200))) correctLane(3);
    if (loop_xPosition >= 2000) turn();
    break;

  case -5:
    if (setCoordTramo(4, 2800, 2200)) correctLane(4);
    if (loop_yPosition <= 2000) changeLane(5);
    break;

  case -6:
    if ((loop_yPosition >= 1400) && (setCoordTramo(5, 2800, 2200))) correctLane(5);
    if (loop_yPosition <= 1000) turn();
    break;

  case -7:
    if (setCoordTramo(6, 200, 800)) correctLane(6);
    if (loop_xPosition <= 2000) changeLane(7);
    break;

  case -8:
    if ((loop_xPosition >= 1400) && (setCoordTramo(7, 200, 800))) correctLane(7);
    if (loop_xPosition <= 1000) turn();
    break;
  ...
  case 2:
    if ((loop_yPosition <= 1600) && setCoordTramo(1, 2200, 2800)) correctLane(1);
    if (loop_yPosition >= 2000) turn();
    if (totalGiros >= 4) {
      isRecognizing = false;
      moveCamera(0);
  ...
  case 3:
    ...
    if (loop_xPosition <= 2000) changeLane(3);
    break;

  case 4:
    if ((loop_xPosition >= 1400) && (setCoordTramo(3, 2200, 2800))) correctLane(3);
    if (loop_xPosition <= 1000) turn();
    break;
  ...
  case 6:
    if ((loop_yPosition >= 1400) && (setCoordTramo(5, 800, 200))) correctLane(5);
    if (loop_yPosition <= 1000) turn();
    break;

  case 7:
    if (setCoordTramo(6, 800, 200)) correctLane(6);
    if (loop_xPosition >= 1000) changeLane(7);
    break;

  case 8:
    if ((loop_xPosition <= 1600) && (setCoordTramo(7, 800, 200))) correctLane(7);
    if (loop_xPosition >= 2000) turn();
    break;
  }
}
...
void calculateCameraAngle(uint16_t bX, uint16_t bY) {
  int _ang = atan3(bX - loop_xPosition, bY - loop_yPosition) * turnSense; // this angle is given with respect to the Y axis, increasing anti-cloclwise -like the IMU angle
...
void autoMoveCamera() {
  ...
  case -3:
    if (loop_xPosition <= 1000 - lidarToImu)
      calculateCameraAngle(1000, 2500);
    else {
  ...
  case -4:
    if (loop_xPosition <= 1500 - lidarToImu)
      calculateCameraAngle(1500, 2500);
    else if (loop_xPosition <= 2000 - lidarToImu)
  ...
  case -5:
    if (loop_yPosition >= 2000 + lidarToImu)
      calculateCameraAngle(2500, 2000);
    else {
  ...
  case -6:
    if (loop_yPosition >= 1500 + lidarToImu)
      calculateCameraAngle(2500, 1500);
    else if (loop_yPosition >= 1000 + lidarToImu)
  ...
  case -7:
    if (loop_xPosition >= 2000 + lidarToImu)
      calculateCameraAngle(2000, 500);
    else {
  ...
  case -8:
    if (loop_xPosition >= 1500 + lidarToImu)
      calculateCameraAngle(1500, 500);
    else if (loop_xPosition >= 1000 + lidarToImu)
  ...
  case -1:
    if (loop_yPosition <= 1000 - lidarToImu)
