...
int exeRANSAC(lidarStorage * storage, Line2D * outputBuffer, Point2D * unassocBuffer) {
    ...
    printf("Data buffer size: %u", unassocPoints.size());
    ...
    while (totalTrials < MAX_TRIALS) {
        ...
        while ((sampleTrials < 2*DEGREE_RANGE*DEGREES_PER_MEASUREMENT) && (sampledPoints < SAMPLE_SIZE))
        {
            ...
            if (isNew) {
                if (abs(storage->getMeasurement(unassocPoints[index]).bearing - centreAngle) < DEGREE_RANGE) { // what if angle 1 is 359º and angle 2 is 1º, the angular distance should be 2º. bearing is set so that the discontinuity is at 180º, an angle that is never measured since the electronics of the car are in the way
                    // accept point
